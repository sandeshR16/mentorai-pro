...
function Admin() {
  const handleExportReport = () => {
    ...
    let exporterName = "Administrator";
    let exporterEmail = "admin@mentorai.com";
    if (currentUserStr) {
      try {
        const u = JSON.parse(currentUserStr);
        exporterName = u.name || exporterName;
        exporterEmail = u.email || exporterEmail;
      } catch (e) {}
    }

    let content = `MENTORAI INSTITUTIONAL ANALYTICS REPORT\n`;
    content += `Generated on: ${new Date().toLocaleString()}\n`;
    content += `Generated by: ${exporterName} (${exporterEmail})\n`;
    content += `=====================================================\n\n`;
    content += `Partner Colleges: 24\n`;
    content += `Active Students: 5,302\n`;
    content += `Verified Offers: 1,420\n`;
    content += `Placement Rate: 92%\n`;
    content += `Partner Companies: 186\n\n`;
    
    content += `SECTOR DISTRIBUTION OF OFFERS:\n`;
    content += `------------------------------\n`;
    sectorData.forEach(s => {
      content += `- ${s.name}: ${s.value}%\n`;
    });
    content += `\n`;
    
    content += `COLLEGE PERFORMANCE:\n`;
    content += `--------------------\n`;
    content += `College Name | Students | Placed | Placement Rate | Avg CTC\n`;
    collegeMetrics.forEach(c => {
      content += `${c.name} | ${c.students} | ${c.placed} | ${c.rate}% | ₹${c.avgCTC} LPA\n`;
    });
    ...
    link.href = url;
    link.download = `MentorAI_Institutional_Analytics_Report_${new Date().toISOString().slice(0,10)}.txt`;
    ...
    <>
      ...
      <div className="p-6 lg:p-8 space-y-6 max-w-[1600px] mx-auto">
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <button onClick={handleExportReport} className="flex items-center gap-2 px-4 py-2 glass-card rounded-lg text-sm text-ink-200 hover:text-white cursor-pointer">
